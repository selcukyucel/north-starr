...
**Preferred:** The `layoutplan` agent (`.claude/agents/layoutplan.md`) runs on a separate thread, keeping the main context clean for coding. `/invert` automatically spawns it after persisting its analysis to `.plans/INVERT-<name>.md`.

**Fallback:** This skill runs in the main context. Use it when:
- The layoutplan agent is not available (not generated by `/bootstrap`)
- You need interactive planning with mid-process user input
- You're running `/layoutplan resume` or `/layoutplan status` to check existing plans
...
**Actions:**
1. Check for a persisted inversion file in `.plans/INVERT-*.md` — this is the primary source (written by `/invert` Step 5)
2. If no file exists, check if `/invert` was run recently in this session (look for inversion analysis output in the conversation)
3. If inversion analysis is available (from file or conversation):
   - Extract risks, edge cases, and failure modes
   - These become constraints and dedicated tasks in the plan
4. If no inversion analysis and the task appears complex:
