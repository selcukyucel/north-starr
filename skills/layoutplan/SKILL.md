...
**Preferred:** The `layoutplan` agent runs on a separate thread, keeping the main context clean for coding. `/invert` automatically spawns it after persisting its analysis to `.plans/INVERT-<name>.md`.

Agent locations by tool:
